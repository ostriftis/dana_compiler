...
class func {
  public:
    ...
    func(DataType t, std::vector<std::tuple<std::string, gen_type *>> p):  type(t), params(p) {}
...
struct STEntry {
  ...
  STEntry(DataType t, std::vector<std::tuple<std::string, gen_type *>> p) {
...
class Scope {
 public:
  ...
  void insert(std::string v, DataType t, std::vector<std::tuple<std::string, gen_type *>> par){
    if (locals.find(v) != locals.end()) yyerror("Duplicate variable declaration: " + v);
...
class SymbolTable {
 public:
  ...
  //Case of function
  void insert(std::string v, DataType t, std::vector<std::tuple<std::string, gen_type *>> p) {
  ...
  public:
    ...
    void break_loop(std::string n = "") {
      if (n == "") {
        ...
        int i = find(n);
        if (i == -1) yyerror("Break statement is used outside of a loop");
        for (int j = loops.size() - 1; j > i; j--) {
          if(loops.back() != "") st.del_loop(loops.back());
          loops.pop_back();
    ...
    void cont_loop(std::string n = "") {
      ...
      std::cout << "Entered Continue loop for: " << n << std::endl;
      if (n == "") {
        if(loops.empty()) yyerror("Continue statement is used outside of a loop");
      }
      else {
        int i = find(n);
        if (i == -1) yyerror("Continue statement is used outside of a loop");
