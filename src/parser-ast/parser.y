...
header:
    T_id                                          {$$ = new Header(std::string($1), nullptr, nullptr);}
|   T_id "is" data_type                           {$$ = new Header(std::string($1), $3, nullptr);}
|   T_id "is" data_type ':' fpar_def header_aux   {$6->insert_front($5); $$ = new Header(std::string($1), $3, $6);}
|   T_id ':' fpar_def header_aux                  {$4->insert_front($3); $$ = new Header(std::string($1), nullptr, $4);}
;
...
|   array_type                                    {$$ = new FparType(nullptr, nullptr, $1);}
;
...
id_aux:
    T_id                                         {$$ = new IdAux(std::string($1));} 
|   id_aux T_id                                  {$1->append(std::string($2)); $$ = $1;} 
;
...
|   "loop" T_id ':' block                        {$$ = new Loop($4, $2);}
|   "break"                                      {$$ = new Break();}
|   "break" ':' T_id                             {$$ = new Break($3);}
|   "continue"                                   {$$ = new Continue();}
|   "continue" ':' T_id                          {$$ = new Continue($3);}
;
...
proc_call:
    T_id                        {$$ = new ProcCall(std::string($1));}
|   T_id ':' expr expr_aux      {$4->insert_front($3); $$ = new ProcCall(std::string($1), $4);}
;
...
func_call:
    T_id '(' ')'                {$$ = new FuncCall($1);}
|   T_id '(' expr expr_aux ')'  {$4->insert_front($3); $$ = new FuncCall($1, $4);}
;

l_value:
    T_id                        {$$ = new L_val(std::string($1), false);}
|   T_string                    {$$ = new L_val(std::string($1), true);}
...
cond_aux:
   "not" cond                   {$$ = new Cond(new UnOp($1, $2));}
|   cond "and" cond             {$$ = new Cond(new BinOp($1, $2, $3));}
|   cond "or" cond              {$$ = new Cond(new BinOp($1, $2, $3));}
|   expr "=" expr               {$$ = new Cond(new BinOp($1, $2, $3));}
|   expr "<>" expr              {$$ = new Cond(new BinOp($1, $2, $3));}
|   expr "<" expr               {$$ = new Cond(new BinOp($1, $2, $3));}
|   expr ">" expr               {$$ = new Cond(new BinOp($1, $2, $3));} 
|   expr "<=" expr              {$$ = new Cond(new BinOp($1, $2, $3));} 
